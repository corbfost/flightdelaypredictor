...
def pos_neg_predict(X_t, prob_right, mod_pos, mod_neg):
    ...
    binom_sample = np.random.binomial(1, prob_right, 50000)
    left_sample = np.random.exponential(yhat_negative, 50000)
    right_sample = np.random.exponential(yhat_positive, 50000)
...
def transform_user_input(origin, dest, airline, year, month, day, time, lookupdf):
    ...
    relpoint = lookupdf.ix[(lookupdf['iata'] == dest) & (lookupdf['UniqueCarrier'] == airline), :]
